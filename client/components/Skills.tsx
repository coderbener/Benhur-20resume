        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {skillCategories.map((skillGroup, index) => (
            <div
              ...
              style={{ borderColor: "rgba(239, 68, 68, 0.3)", background: "rgba(20, 20, 20, 0.5)" }}
              onMouseEnter={(e) => {
            ...
            >
              {/* Category Title */}
              <h3 className="text-2xl sm:text-3xl font-bold text-white mb-8 relative z-10 group-hover:text-red-300 transition-colors">
                {skillGroup.category}
              </h3>
              ...
              <ul className="space-y-4 relative z-10">
                {skillGroup.skills.map((skill, skillIndex) => (
                  ...
                  >
                    <span className="text-red-500 text-lg font-bold group-hover/item:text-red-400 transition-colors">✓</span>
                    <span className="group-hover/item:text-red-300 transition-colors">{skill}</span>
                  </li>
                ))}
              </ul>
            </div>
